...
export const Sellermyproductpage = () => {
  ...
  const sellerRaw = localStorage.getItem("sellerUser") || localStorage.getItem("sellerProfile");
  let sellerName = "Seller";
  if (sellerRaw) {
    try {
      const parsed = JSON.parse(sellerRaw) as { name?: string; fullName?: string };
      sellerName = parsed.name || parsed.fullName || "Seller";
    } catch {
      sellerName = "Seller";
    }
  }
  ...
  const openEdit = (product: (typeof myProducts)[number]) => {
    navigate(`/admin/seller/products/edit/${product._id}`, { state: { product } });
  };
 ...
 <Paper sx={{ p: 2.5, borderRadius: 2.5, border: "1px solid var(--skote-border)", boxShadow: "none" }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
          ...
          <Typography variant="caption" color="var(--skote-subtle)">Logged in seller: {sellerName}</Typography>
        </Stack>
